...
export function SpeciesSearchClient({ groups }: SpeciesSearchClientProps) {
  ...
  const filtered = groups.filter((group) =>
    search === "" ||
    group.representative.name.toLowerCase().includes(search.toLowerCase()) ||
    (group.representative.habitat || "").toLowerCase().includes(search.toLowerCase()) ||
    (group.representative.shape || "").toLowerCase().includes(search.toLowerCase())
  );
    ...
    <>
      <header className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <h1 className="text-3xl font-semibold text-gray-900">
          Species Codex ({filtered.length} species)
        </h1>
        ...
        <input
          type="text"
          placeholder="Search by name, habitat, or shape..."
          value={search}
